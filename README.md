...
### GitHub Actions approvals for forked pull requests

- When contributors open pull requests from forks, GitHub may require a maintainer with **write access** to manually approve the workflow run before it executes. Approve these runs from the **Actions** tab to let the `pages-deploy.yml` workflow proceed.
- You can configure workflow approval requirements at the repository level under **Settings → Actions → General → Workflow approvals**, or centrally at the organization/enterprise level if you manage policies there.
