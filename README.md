...
- `OPENAI_API_KEY` → your OpenAI API key (required for AI sales roleplay)
...
- The `OPENAI_API_KEY` is server-side only — it never reaches the browser; store it as a Vercel secret and never expose it in client-side code
