...
### GitHub Repo Consolidation (single main repo)

Keep `fontan-sales-association` as the only main GitHub repository, merge the old repository into it, then point Vercel to that one repo only.

```bash
# 1) Clone the main repo
git clone https://github.com/OWNER/fontan-sales-association.git
cd fontan-sales-association

# 2) Add the old repo as a remote
git remote add oldrepo https://github.com/OWNER/fsa-elite-performance.git
git fetch oldrepo

# 3) Merge old repo history into main repo (preferred)
...
git status
git add .
git commit -m "Merge FSA Elite Performance into Fontan Sales Association"

# 5) Push merged result
git push origin main
```

If you do not want mixed history, copy files instead:

```bash
git clone https://github.com/OWNER/fontan-sales-association.git
cd fontan-sales-association
git clone https://github.com/OWNER/fsa-elite-performance.git temp-old-repo
cp -R temp-old-repo/* .
cp -R temp-old-repo/.* . 2>/dev/null || true
rm -rf temp-old-repo
git add .
git commit -m "Move FSA Elite Performance into main repo"
git push origin main
```

After GitHub migration, open **Vercel → Project → Settings → Git** and confirm the project is connected to `OWNER/fontan-sales-association` only.
