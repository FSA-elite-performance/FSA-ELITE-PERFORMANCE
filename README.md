...
npm ci
npx --yes vercel link        # first time only
npm run env:pull             # pulls the latest Vercel env vars into .env.local
npm run dev
```
...
Pull the latest local environment variables from Vercel before starting the app:
...
If the project has not been linked to Vercel on your machine yet, run `npx --yes vercel link` first.
