...
### Create & trigger a Vercel Deploy Hook

Deploy Hooks let you re-run the Vercel build for a specific branch by sending a
simple `GET` or `POST` request to a unique URL. This is useful when you want to:

- redeploy after content changes from a CMS
- force a fresh production build from the command line
- trigger scheduled rebuilds from a cron service

To create one for this project:

1. Make sure the Vercel project is connected to this GitHub repository
2. In Vercel, open **Project → Settings → Git**
3. In **Deploy Hooks**, choose a clear name such as `Production Content Rebuild`
4. Select the branch you want the hook to deploy
5. Copy the generated hook URL and store it somewhere secure

To trigger the hook, send a request to the URL — no auth header or request body is required:

```bash
curl -X POST "https://api.vercel.com/v1/integrations/deploy/<project-id>/<deploy-hook-id>"
```

You can also keep the hook URL in a local shell variable and trigger it on demand:

```bash
export VERCEL_DEPLOY_HOOK_URL="https://api.vercel.com/v1/integrations/deploy/<project-id>/<deploy-hook-id>"
curl -X POST "$VERCEL_DEPLOY_HOOK_URL"
```

**Important notes**

- Treat the Deploy Hook URL like a password — anyone with it can trigger a deployment
- To skip Vercel's existing build cache for a one-off rebuild, append `?buildCache=false`
- Deploy Hooks do not run if `github.enabled = false` is set in `vercel.json`
- This repository does not currently include a `vercel.json`, so there is no local config blocking Deploy Hooks by default
...
> **Note:** GitHub Pages must be enabled in **Settings → Pages** before the deploy job can succeed. The included workflow already sets `NEXT_EXPORT=1`. You do not need to uncomment `output: 'export'` manually in `next-app/next.config.js`.
