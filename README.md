...
AI prompts used by the platform are stored as `.prompt.yml` files in the `prompts/` directory so they can be browsed, shared, and iterated on directly in GitHub.

| File | Purpose |
|------|---------|
| `prompts/olive-assistant.prompt.yml` | General OLIVE assistant — site-wide sales coaching and platform guidance |
| `prompts/olive-roleplay.prompt.yml` | OLIVE Roleplay Lab — customer simulation and coaching feedback |

Each file follows the [GitHub `.prompt.yml` format](https://docs.github.com/en/copilot/customizing-copilot/using-prompt-files) with model parameters, system/user message templates using `{{variable}}` placeholders, sample test data, and evaluators.
