...
### `terminal sandbox snapshots delete` [OPTIONS]

Delete a saved sandbox snapshot using the Terminal CLI.

> **Warning:** Deleting a snapshot is permanent and cannot be undone.

```bash
terminal sandbox snapshots delete <snapshot-id> [--sandbox <name>] [--yes] [--help]
```

**Options**
- `<snapshot-id>` (required) — the snapshot identifier to delete.
- `--sandbox <name>` — scope deletion to a specific sandbox when you have multiple configured.
- `-y, --yes` — skip the interactive confirmation prompt (use with care).
- `-h, --help` — show help for this command.

**Example**

```bash
# Remove snapshot 123 from the default sandbox without a prompt
terminal sandbox snapshots delete 123 --yes
```
