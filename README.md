...
| `NEXT_PUBLIC_SITE_URL` | Full URL of your deployment (e.g. `https://fsaeliteperformance.com` or `http://localhost:3000`) |
...
- `NEXT_PUBLIC_SITE_URL` → `https://fsaeliteperformance.com`
- `STRIPE_SECRET_KEY` → your Stripe secret key
- `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` → your Stripe publishable key

**Training project (fsaelite.org):**
- `NEXT_PUBLIC_SITE_URL` → `https://fsaelite.org`
...
- `NEXT_PUBLIC_SITE_URL` → your production domain (e.g. `https://fsaeliteperformance.com`)
...
3. Add `NEXT_PUBLIC_SITE_URL` as a GitHub repository variable (Settings → Variables)
...
```bash
npx ngrok http 3000
# Copy the https://... ngrok URL and set it as NEXT_PUBLIC_SITE_URL in .env.local
```
