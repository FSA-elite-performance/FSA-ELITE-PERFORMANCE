...
The Vercel Sandbox CLI uses a Docker-like command style to manage sandboxes, run
commands, copy files, and debug directly from your terminal. For this project, treat
it as an optional contributor tool for manual testing/debug workflows (it is not
required for the standard `next-app` build/deploy flow). You can also use the SDK to
automate sandbox workflows in your application.
...
You can invoke it with either `sandbox` or `sbx`.
...
Show available Sandbox CLI commands:
