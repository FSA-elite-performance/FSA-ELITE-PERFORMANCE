...
| `STRIPE_WEBHOOK_SECRET` | Webhook signing secret for `/api/webhook` (`whsec_...`) — see [Webhook Setup](#-stripe-webhook-setup) |
...
The `/api/webhook` endpoint verifies every payment Stripe processes and is **required** in production.
Without it the store accepts payments but cannot reliably confirm them or provision access.

### How it works

1. Stripe calls `POST /api/webhook` after a successful payment.
2. The handler verifies the request using a **webhook signing secret** (`STRIPE_WEBHOOK_SECRET`) to ensure it really came from Stripe.
3. On a `checkout.session.completed` event the purchase is logged and access can be provisioned (email, database record, etc.).

### Production setup (Vercel)

1. Deploy the app to Vercel (see [Deployment](#-deployment)).
2. Go to [dashboard.stripe.com/webhooks](https://dashboard.stripe.com/webhooks) → **Add endpoint**.
3. Set the endpoint URL to `https://fsaeliteperformance.com/api/webhook`.
...
6. In your Vercel project → **Settings → Environment Variables**, add:
   - `STRIPE_WEBHOOK_SECRET` → the `whsec_...` value (mark as **Sensitive**)
7. Redeploy (or Vercel picks up the new env automatically on next push).
...
# The CLI prints a signing secret — add it to .env.local:
...
Check your terminal for the `[webhook] Payment received` log line to confirm it's working.
...
- [x] Add Stripe **webhook endpoint** (`/api/webhook`) to verify payment completion and provision access
