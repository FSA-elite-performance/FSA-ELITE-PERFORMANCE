...
export default function Roleplay() {
    ...
    try {
      const res = await fetch('/api/ai-chat', {
        ...
        body: JSON.stringify({
          messages: trimmed,
          scenarioContext: {
            personaName: getPersonaById(activePersonaId).label,
            goal: getPersonaById(activePersonaId).goal,
            fear: getPersonaById(activePersonaId).fear,
            constraint: getPersonaById(activePersonaId).constraint,
            triggerPhrases: getPersonaById(activePersonaId).triggerPhrases,
            decisionStyle: getPersonaById(activePersonaId).decisionStyle,
            proofNeeded: getPersonaById(activePersonaId).proofNeeded,
            hiddenVariables: getPersonaById(activePersonaId).hiddenVariables,
            patienceMeter: nextPatience,
            bossLevel: getPersonaById(activePersonaId).bossLevel,
          },
        }),
      });
  ...
  function ensureThreeCompletedSessions() {
    setProgress((prev) => ({
      ...prev,
      sessionsCompleted: Math.max(prev.sessionsCompleted, 3),
    }));
  }
        ...
        <section className="roleplay-core">
          ...
          <aside className="roleplay-intel-panel">
            ...
            <div className="roleplay-intel-block">
              ...
              {progress.sessionsCompleted < 3 && (
                <button type="button" onClick={ensureThreeCompletedSessions} className="btn-secondary">
                  Set sessions to 3 completed
                </button>
              )}
            </div>
          </aside>
