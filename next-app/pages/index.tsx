...
const TRAINING_MATERIALS = [
  {
    title: 'Daily Usage Loop',
    body: 'Use the morning, midday, and night prompts to drive action, stop drift, and move more deals every day.',
  },
  {
    title: 'AI Coach Logic',
    body: 'Pressure-based coaching triggers that call out low activity, weak follow-up, and stalled closes in real time.',
  },
  {
    title: 'Performance Metrics',
    body: 'Track killer metrics like response speed, follow-up gaps, ghost rate, and deal stagnation time.',
  },
  {
    title: 'Pipeline Rules & Questions',
    body: 'Simple rules and hard qualifying questions to identify who is serious, who is stalling, and where to apply pressure.',
  },
  {
    title: 'Sales Code & Identity System',
    body: 'Core operator principles that reinforce certainty, direct asks, close attempts, and consistent production behavior.',
  },
];
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const message =
          res.status === 404
            ? formatStoreError('Checkout is unavailable in this static preview')
      ...
      if (err instanceof TypeError) {
        setError(formatStoreError('Checkout service is currently unavailable'));
      } else {
      ...
      <main style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            >
              {FEATURES.map((f) => (
                ...
                >
                  ...
                  <p style={{ color: '#aaa', fontSize: '0.92rem', lineHeight: 1.65 }}>{f.body}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={{ padding: '5rem 1.5rem', background: '#111' }}>
          <div className="container">
            ...
            >
              Sales Training Materials Included
            </h2>
            <p
              style={{
                textAlign: 'center',
            ...
            >
              Practical frameworks from the FSA Elite operator system — built for real showroom pressure.
            </p>
            ...
            >
              {TRAINING_MATERIALS.map((material) => (
                ...
                >
                  ...
                  <p style={{ color: '#aaa', fontSize: '0.92rem', lineHeight: 1.65 }}>{material.body}</p>
                </div>
              ))}
            </div>
          </div>
