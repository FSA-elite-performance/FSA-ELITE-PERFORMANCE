...
// ─── Handler ──────────────────────────────────────────────────────────────────
export default async function createCheckoutSessionHandler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') {
  ...
  const stripeApiClient = getStripeClient(secretKey);

  try {
    const checkoutSession = await stripeApiClient.checkout.sessions.create({
    ...
    const errorMessage =
      error instanceof Stripe.errors.StripeError ? error.message : 'Internal server error';
