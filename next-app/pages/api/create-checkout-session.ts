...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const verification = await verifyBotIdRequest(req);

    if (verification.isBot) {
      return res.status(403).json({ error: 'Access denied.' });
    }
  } catch (err: unknown) {
    console.error('BotID verification error:', err);
    return res.status(500).json({ error: 'Bot verification unavailable. Please try again later.' });
  }
