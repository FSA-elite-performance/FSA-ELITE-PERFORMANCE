...
function isAllowedVerifiedBot(botVerification: BotVerification): boolean {
  if (!botVerification.isVerifiedBot) {
    return false;
  }

  const verifiedBotName = botVerification.verifiedBotName?.trim().toLowerCase();
  const verifiedBotCategory = botVerification.verifiedBotCategory?.trim().toLowerCase();
...
export default async function handler(
  ...
  try {
    ...
    if (botVerification.isBot && !isAllowedVerifiedBot(botVerification)) {
