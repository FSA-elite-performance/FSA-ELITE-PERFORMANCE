...
import { AI_CHAT_MAX_MESSAGES, AI_CHAT_MAX_MSG_CHARS, ChatMessage } from '../../lib/aiChatConstants';
...
export default async function handler(
  ...
  if (messages.length > AI_CHAT_MAX_MESSAGES) {
    return res
      .status(400)
      .json({ error: `Too many messages. Maximum is ${AI_CHAT_MAX_MESSAGES} per request.` });
  }
  ...
  for (const msg of messages) {
    ...
    if (msg.content.length > AI_CHAT_MAX_MSG_CHARS) {
      return res
        .status(400)
        .json({ error: `Message content exceeds ${AI_CHAT_MAX_MSG_CHARS} character limit.` });
    }
  }
