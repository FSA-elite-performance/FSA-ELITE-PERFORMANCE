...
- `NEXT_PUBLIC_SANITY_DATASET` — Sanity dataset name (client-safe, default `production`)
- `SANITY_PROJECT_ID` — Sanity project ID (server-side)
- `SANITY_DATASET` — Sanity dataset name (server-side, default `production`)
- `SANITY_API_READ_TOKEN` — **Secret** read-only API token
- `SANITY_API_WRITE_TOKEN` — **Secret** read-write API token
