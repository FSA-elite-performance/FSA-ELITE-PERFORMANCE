...
import { getTodayLesson, TRAINING_TOPICS, type Lesson } from '../lib/trainingPath';
  ...
  return (
    <>
      ...
      <main className="dash-page">
        ...
        {/* Today's Lesson Card */}
        {todayLesson && (() => {
          const topic = TRAINING_TOPICS.find((t) => t.id === todayLesson.topicId);
          return (
            <div className="dash-today-lesson">
              <div className="dash-today-lesson-meta">
                <span className="dash-today-lesson-eyebrow">
                  {topic?.icon ?? '📚'} {topic?.title ?? 'Training'} · Day {todayLesson.day}
                </span>
                <h2 className="dash-today-lesson-title">{todayLesson.title}</h2>
                <p className="dash-today-lesson-concept">
                  {todayLesson.concept.split('.')[0]}.
                </p>
              </div>
              <div className="dash-today-lesson-actions">
                <span className="dash-today-lesson-duration">⏱ {todayLesson.duration}</span>
                <Link href="/training" className="btn-primary btn-sm">
                  Start Lesson →
                </Link>
